...
public class ILog {
	...
	private static ILog c_Instance;
...
namespace MarkovSharp {
	...
	public abstract class GenericMarkov<TPhrase, TUnigram> : IMarkovStrategy<TPhrase, TUnigram> {
		...
		private readonly ILog Logger = ILog.Instance;

		protected GenericMarkov ( int level = 2 ) {
			if ( level < 1 ) {
				throw new ArgumentException ( "Invalid value: level must be a positive integer", "int level" );
			}
		...
		public Type UnigramType {
			get {
				return typeof ( TUnigram );
			}
		}
		public Type PhraseType {
			get {
				return typeof ( TPhrase );
			}
		}
		...
		public void Learn ( TPhrase phrase ) {
			Logger.Info ( string.Format ( "Learning phrase: '{0}'", phrase ) );
			...
			if ( SplitTokens ( phrase ).Count () < Level ) {
				Logger.Info ( string.Format ( "Phrase {0} too short - skipped", phrase ) );
				return;
			}
			...
			if ( !SourcePhrases.Contains ( phrase ) ) {
				Logger.Debug ( string.Format ( "Adding phrase {0} to source lines", phrase ) );
			...
			for ( var j = Level; j > 0; j-- ) {
				...
				try {
					...
					Logger.Debug ( string.Format ( "Adding TGram ({0}) {1} to lastCol", typeof ( TUnigram ), previous ) );
					lastCol.Add ( previous );
				}
				catch ( IndexOutOfRangeException e ) {
					Logger.Warn ( string.Format ( "Caught an exception: {0}", e ) );
					previous = GetPrepadUnigram ();
					lastCol.Add ( previous );
				}
			}

			Logger.Debug ( string.Format ( "Reached final key for phrase {0}", phrase ) );
		...
		/// </summary>
		/// <param name="tokens"></param>
		private void LearnTokens ( TUnigram[] tokens ) {
		...
		public void Retrain ( int newLevel ) {
			if ( newLevel < 1 ) {
				throw new ArgumentException ( "Invalid argument - retrain level must be a positive integer", "int newLevel" );
			}

			Logger.Info ( string.Format ( "Retraining model as level {0}", newLevel ) );
			Level = newLevel;
